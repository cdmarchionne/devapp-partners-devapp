...
public class PlanService implements Serializable {
    ...
    public PlanDto findById(final Serializable id) throws NoHayResultadoException { // NOPMD
        PlanDeCarrera plan = planDao.findById(id);
        if (plan == null) { throw new NoHayResultadoException(); }
        return PlanUtilsDto.objectToDto(plan);// NOPMD
    }
